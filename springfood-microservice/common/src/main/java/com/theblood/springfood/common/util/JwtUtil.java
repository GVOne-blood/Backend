...
public class JwtUtil {
    ...
    // Ưu tiên đọc từ config yaml (app.jwt.secretKey)
    // Nếu không có, đọc trực tiếp từ biến môi trường (JWT_SECRET_KEY)
    // Nếu cả 2 không có, giá trị là null (tránh crash lúc startup cho các service không dùng)
    @Value("${app.jwt.secretKey:${JWT_SECRET_KEY:#{null}}}")
    private String secretKey;
    @Value("${app.jwt.refreshKey:${JWT_REFRESH_KEY:#{null}}}")
    private String refreshKey;
    @Value("${app.jwt.resetKey:${JWT_RESET_KEY:#{null}}}")
    private String resetKey;

    @javax.annotation.PostConstruct
    public void init() {
        if (secretKey == null) {
            log.warn("JWT Secret Key is missing! JwtUtil signatures will fail if used.");
        } else {
            log.info("JwtUtil initialized with secret key present.");
        }
    }
    ...
    private Key getSigningKey(TokenType tokenType) {
        ...
        switch (tokenType) {
            case ACCESS:
                if (secretKey == null) throw new IllegalStateException("JWT Secret Key is not configured");
