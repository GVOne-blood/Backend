...
## [2. Có thể chuyển đổi giữa hai kiểu dữ liệu này không ?](#chuyển-đổi-giữa-2-loại-dữ-liệu)

## [3. Có thể so sánh hai kiểu dữ liệu này với nhau không?](#phương-thức-so-sánh-primitive-và-object)

## [4. Giá trị khi khởi tạo biến với hai loại kiểu dữ liệu này là gì?](#giá-trị-mặc-định)
...
- Cấu trúc cây phân cấp của Object Data Type:
  - java.lang.Object : Là root class của hệ thống phân cấp, mọi lớp đều là lớp con của Object, nghĩa là mọi class đều kế thừa từ lớp này và cố các phương thức cơ bản của object như _equals(), hashCode(), toString(), getClass(), ..._
...
- Wrapper class: Mục đích Wrapper ra đời để phục vụ bản chất hương đối tượng của Java - Mọi thứ đều có thể được thể hiện dưới dạng đối tượng với các thuộc tính và phương thức. Một số các Collections framework chỉ hoạt động với Object datatype như Map, Set,...Ngoài ra primitive không thể biểu diễn trạng thái "không có giá trị" của biến.

- Cơ chế Boxing và Unboxing: Bản chất của boxing là việc thay đổi giá trị của biến value trong đối tượng bao bọc nó, unboxing thì ngược lại, ví dụ về 1 lớp Wrapper int:
  ...
  một lớp wrapper thực chất chứa một biến private final + kiểu nguyên thủy tương ứng. Vì vậy muốn boxing một biến primitive ta có thể gọi constructor, method valueOf(), mục đích cuối cùng để gán giá trị cho biến value. Tương tự với boxing, Java cung cấp các phương thức accessor để trích xuất primitive value ra ngoài (Unboxing):

  ![box](https://github.com/GVOne-blood/Backend/blob/main/demo/src/main/resources/local/Screenshot%202025-08-25%20151007.png)

---

## Phương thức so sánh Primitive và Object

> Về bản chất, ta không thể so sánh một primitive value với một reference value

- Xét ví dụ :

  ![vd](https://github.com/GVOne-blood/Backend/blob/main/demo/src/main/resources/local/Screenshot%202025-08-25%20153734.png)

  bản chất, y lưu trữ địa chỉ ô nhớ trỏ đến đối tượng Integer trong Heap, x lưu trữ giá trị nhị phân trong Stack, vì vậy về lý thuyết việc so sánh **x == y** sẽ cho ra false, nhưng thực tế lại trả về true do Java đã can thiệp trong quá trình biên dịch để unboxing cho biến Object. Đối với **y.equals(x)**, bản chất phương thức equals mặc định đã boxing x khiến kết quả trả về true

## Giá trị mặc định

> number: 0, 0d(double), 0f(float)
...
- Các biến cục bộ đều không được khởi tạo bất kỳ giá trị nào bất chấp kiểu dữ liệu, Java sẽ nhả một lỗi biên dịch để thông báo việc này, vì vậy các biến trong hàm, phương thức, người dùng đều phải khởi tạo giá trị cho chúng
- Các phần tử của mảng khi được khởi tạo bằng từ khóa new và được cấp phát bộ nhớ trên Heap, giá trị mặc định của chúng sẽ là giá trị mặc định của kiểu dữ liệu của mảng

  ![sam](https://github.com/GVOne-blood/Backend/blob/main/demo/src/main/resources/local/Screenshot%202025-08-25%20160141.png)
